...
export default function SubSectionModal({
  ...
  // Prefill form when in view/edit mode
  useEffect(() => {
    if (view || edit) {
  ...
  // Check if the form actually changed during edit
  const isFormUpdated = () => {
  ...
  const handleEditSubsection = async () => {
    ...
    if (result) {
      // Update course content immutably
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (data.lectureVideo && typeof data.lectureVideo !== "string") {
        duration = await getVideoDuration(data.lectureVideo);
      }
  ...
  return (
    <div className="fixed inset-0 z-[1000] grid h-screen w-screen place-items-center bg-white/10 backdrop-blur-sm overflow-auto">
...
//       formData.append("title", data.lectureTitle);
//       formData.append("description", data.lectureDesc);
//       formData.append("video", data.lectureVideo); // must be "video"
//       formData.append("timeDuration", duration);
